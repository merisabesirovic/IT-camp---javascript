...
function prosek(niz) {
  const filtriran = niz.filter((el) => el.Director === "Christopher Nolan");
  const duzina = filtriran.length;
  const prosek =
    filtriran.reduce((acc, movie) => acc + parseFloat(movie.imdbRating), 0) /
    duzina;
  return prosek;
}
